...
func.func @test_sigmoid_diff_rank(%arg0 : tensor<?xf32>) -> tensor<?x?xf32> {
  // expected-error@+1{{'tcp.sigmoid' op all non-scalar operands/results must have the same shape and base type}}
  %0 = tcp.sigmoid %arg0 : tensor<?xf32> -> tensor<?x?xf32>
  return %0 : tensor<?x?xf32>
}

// -----

func.func @test_sigmoid_diff_shape(%arg0 : tensor<5x?xf32>) -> tensor<6x?xf32> {
  // expected-error@+1{{'tcp.sigmoid' op all non-scalar operands/results must have the same shape and base type}}
  %0 = tcp.sigmoid %arg0 : tensor<5x?xf32> -> tensor<6x?xf32>
  return %0 : tensor<6x?xf32>
}

// -----

func.func @test_sigmoid_diff_type(%arg0 : tensor<?x?xf16>) -> tensor<?x?xf32> {
  // expected-error@+1{{'tcp.sigmoid' op requires the same element type for all operands and results}}
  %0 = tcp.sigmoid %arg0 : tensor<?x?xf16> -> tensor<?x?xf32>
  return %0 : tensor<?x?xf32>
}